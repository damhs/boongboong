...
function SearchPlace() {
  ...
  const handleSelect = async (selectedPlace) => {
    ...
    try {
      ...
      var existingPlace = places.data.find(
        (place) => place.placeName === title
      );
      if (existingPlace) {
      ...
      var recentEntry = recents.data.find(
        (recent) => recent.placeID === existingPlace.placeID
      );
      ...
      // recents에 추가
      const postRecent = async (recentData) => {
        try {
          const response = await axios.post(`${baseurl}/recents`, recentData);
          console.log("Recent added:", response.data);
        } catch (error) {
      ...
      >
        <div className={styles.inputContainer}>
          <input
            type="text"
            placeholder={
              placeType === "departure" ? "출발지 입력" : "도착지 입력"
            }
